...
const manageSubscriptionStatusChange = async (subscriptionId: string, customerId: string) => {
  ...
  const { data: customerData } = await supabase.from('customers').select('establishment_id').eq('stripe_customer_id', customerId).single();
  if (!customerData) throw new Error('Customer not found');
  const { establishment_id } = customerData;
