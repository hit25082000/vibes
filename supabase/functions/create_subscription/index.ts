...
async function getCustomerFromestablishmentId(establishment_id: string) {
  const { data, error } = await supabase.from('customers').select('*').eq('establishment_id', establishment_id).maybeSingle();
  if (error) throw new Error(error.message);
  return data;
}

async function createOrRetrieveCustomer(stripeClient: any, establishment_id: string, email: string, name: string, phone: string): Promise<iCustomer> {
  const customer = await getCustomerFromestablishmentId(establishment_id);
  if (customer) return customer;
  if (!establishment_id) throw new Error('O ID da empresa é obrigatório');
...
serve(async req => {
  ...
  try {
    ...
    const { establishment_id, price_id, name, email, phone }: iSubscribeRequest = await req.json();
    if (!establishment_id) throw new Error('É necessário informar o ID da empresa');
    if (!price_id) throw new Error('É necessário informar o ID do preço');

    const customer = await createOrRetrieveCustomer(stripeClient, establishment_id, email, name, phone);
